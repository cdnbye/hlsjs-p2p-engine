...
class BTTracker extends EventEmitter {
    constructor(channel, config, info) {
        ...
        this.failedDCMap = new Map();                            //{key: remotePeerId, value: boolean} 建立连接失败的dc
        this.signalerWs = null;                                  //信令服务器ws
        this.heartbeatInterval = 30;
        //tracker request API
        this.fetcher = new Fetcher(config.key, sha1.sync(channel), config.announce);
        let queryStr = `?key=${window.encodeURIComponent(config.key)}&info_hash=${sha1.sync(channel)}`;
        this.announceURL = config.announce + '/announce/' + queryStr;
        this.heartbeatURL = config.announce + '/heartbeat/' + queryStr;
        this.getPeersURL = config.announce + '/get_peers/' + queryStr;
        this.statsURL = config.announce + '/stats/' + queryStr;
        /*
