...
class P2PScheduler extends EventEmitter {
    constructor(config) {
        ...
        //fastmesh
        this.totalUploadBW = 1024*1024*4*8;               //总上行带宽默认4Mbps
        this.residualBW = this.totalUploadBW;             //初始剩余带宽等于总上行带宽
        this.streamingRate = 0;                           //单位bps
        this.tranInspectorId = this._setupTransInspector();
    ...
    get uploadStreamingRate() {                          //目前所有datachannel上行streaming rate
        let br = 0;
    ...
    _setupTransInspector() {                          //定时检查是否需要跃迁
        return window.setInterval(() => {
            this.residualBW = this.totalUploadBW - this.uploadStreamingRate;
            if (this.residualBW > this.totalUploadBW/2) {         //如果剩余带宽大于streaming rate
                const msg = {
                    event: 'TRANSITION',
