...
export default class P2PSignaler extends EventEmitter {
    ...
    _handleParentsMsg(nodes) {
        ...
        while (this.failedDCMap.get(parentId)) {             //过滤掉已经连接失败的节点)
        ...
        datachannel.on('signal', data => {
            ...
            .once('error', () => {
                ...
                if (this.candidateParents.length > 0) {                            //如果连接失败与剩余的尝试连接
                    ...
                    if (datachannel.isReceiver && this.scheduler.upstreamers.length < this.config.minParents && this.getParentsTimes <= this.config.maxGetParentsTries) {
                        let msg = {
                            action: 'get_parents'
                        };
                        this.schedulerWs.send(msg);
            ...
            .once(Events.DC_OPEN, () => {
                ...
                if (this.scheduler.transitioning) {                            //跃迁中
                    ...
                    if (this.candidateParents.length > 0 && this.scheduler.upstreamers.length < this.config.maxParents) {
                        let parentId = this.candidateParents.pop().peer_id;
                        this._createDatachannel(parentId, true);
